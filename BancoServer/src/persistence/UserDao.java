...
public class UserDao {
	...
	/**
	 * Constructor for Dao. This constructor will only reset cache. If extended Dao
	 * classes are generated, it is important to make sure resetCache() will be
	...
	/**
	 * resetCache-method. This is important method when caching is used to keep data
	 * in Dao instance. This method must be called whenever the data in table has
	 * been changed. This method will mark current cache to be outdated and next
	 * time when cacheable data will be retrieved from database, the cache will be
	 * rebuilt. Please note that using Dao-cache can have remarkable performace
	 * boost or it may not help at all. It all depends on the amount of data and the
	 * rate that data is being changed.
	 */
	...
	/**
	 * LoadAll-method. This will read all contents from database table and build an
	 * Vector containing valueObjects. Please note, that this method will consume
	...
	public synchronized void create(Connection conn, User valueObject) throws SQLException {
		...
		try {
			sql = "INSERT INTO tbl_usuario (USU_NOMBRE,USU_CEDULA,USU_LOGIN) VALUES(?, ?, ?) ";
			stmt = conn.prepareStatement(sql);
	 ...
	 *                    Primary-key field must be set for this to work properly.
	 */
	public void save(Connection conn, User valueObject) throws NotFoundException, SQLException {

		String sql = "UPDATE tbl_usuario SET USU_NOMBRE = ?, USU_LOGIN = ? WHERE (USU_CEDULA = ? ) ";
		PreparedStatement stmt = null;
	 ...
	 *                    Primary-key field must be set for this to work properly.
	 */
	public void delete(Connection conn, User valueObject) throws NotFoundException, SQLException {

		String sql = "DELETE FROM tbl_usuario WHERE (USU_CEDULA = ? ) ";
		PreparedStatement stmt = null;
	 ...
	 * The result will be 0-N objects in vector, all matching those criteria you
	...
	public List searchMatching(Connection conn, User valueObject) throws SQLException {
		...
		boolean first = true;
		StringBuffer sql = new StringBuffer("SELECT * FROM tbl_usuario WHERE 1=1 ");

		if (valueObject.getId() != 0) {
			if (first) {
				first = false;
			}
			sql.append("AND USU_CEDULA = ").append(valueObject.getId()).append(" ");
		}
		if (valueObject.getName() != null) {
			if (first) {
				first = false;
			}
			sql.append("AND USU_NOMBRE LIKE '").append(valueObject.getName()).append("%' ");
		}
		if (valueObject.getLogin() != null) {
			if (first) {
				first = false;
			}
			sql.append("AND USU_LOGIN LIKE '").append(valueObject.getLogin()).append("%' ");
		}
	...
	protected void singleQuery(Connection conn, PreparedStatement stmt, User valueObject)
		...
		try {
			...
			if (result.next()) {
				...
				// System.out.println("Tbl_usuario Object Not Found!");
				throw new NotFoundException("Tbl_usuario Object Not Found!");
			}
		} finally {
			if (result != null)
				result.close();
